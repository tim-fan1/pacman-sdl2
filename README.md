...
- How to implement variable speeds for PACMAN and ghosts? For example, it would be nice if once PACMAN eats a power pellet, that frightened ghosts start moving slower than they usually do, so that PACMAN can easily catch up to frightened ghosts and send them to their home base.
  ...
  This could be implemented by using pre-turn and post-turn pixels on each tile that would allow PACMAN and the ghosts to more easily navigate around corners — this is discussed here in the [PACMAN Dossier](https://pacman.holenet.info/#CH2_Cornering), which is also where many of the decisions made for this clone were taken from too — so that when the speed changes for PACMAN or the ghosts, they will for any speed in the game likely land on one of these pre-turn or post-turn pixels, so that for any speed in the game they can still successfully navigate corners.   
